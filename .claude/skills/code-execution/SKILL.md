...
allowed-tools: Bash
description: Use when you need to execute Python code locally with marketplace API access for bulk operations (10+ files), complex multi-step workflows, or iterative processing to achieve significant token savings.
hooks:
  Stop:
    - hooks:
        - type: command
...
```bash
bash .claude/hooks/log-lifecycle.sh skill executing-python-code instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
