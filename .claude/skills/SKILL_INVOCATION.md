...
**Skills are automatically hot-reloaded** when created or modified:

- Skills in `~/.claude/skills` (personal) are immediately available
- Skills in `.claude/skills` (project) are immediately available
- No session restart required - changes take effect instantly
- Enables rapid iteration during skill development and testing

---

## Skill Execution Context

Skills can specify their execution context using YAML frontmatter:

### Fork Context (`context: fork`)

Use `context: fork` in skill frontmatter to run the skill in a forked sub-agent context:

```yaml
---
name: isolated-analysis
description: Use when performing analysis that requires isolated state
context: fork
---
```

**When to use fork context:**
- Skill requires isolated state (won't affect main conversation)
...
- Memory-efficient for exploratory operations

### Agent Type Specification (`agent`)

Use the `agent` field to specify which agent type should execute the skill:

```yaml
---
name: codebase-explorer
description: Use when exploring codebase structure and patterns
context: fork
agent: Explore
---
```
...
description: Use when performing comprehensive code review across multiple files
context: fork
