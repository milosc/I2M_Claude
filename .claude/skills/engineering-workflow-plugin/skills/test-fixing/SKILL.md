...
description: Use when you need to systematically identify, group, and fix failing tests in a test suite.
hooks:
  Stop:
    - hooks:
        - type: command
...
```bash
bash .claude/hooks/log-lifecycle.sh skill fixing-failing-tests instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
