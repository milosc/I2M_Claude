...
allowed-tools: Bash, Edit, Grep, Write
description: Use when you need to systematically process and implement changes based on code review feedback, PR comments, or reviewer notes.
hooks:
  Stop:
    - hooks:
        - type: command
...
```bash
bash .claude/hooks/log-lifecycle.sh skill implementing-review-feedback instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
