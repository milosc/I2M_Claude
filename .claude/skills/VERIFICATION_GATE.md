...
VERIFICATION_GATE:
  ...
  1. IDENTIFY: What command(s) prove this phase succeeded?
     - **Discovery Guardrail**: `python3 .claude/hooks/discovery_quality_gates.py --validate-checkpoint <N>`
     - **File Validation**: `python3 .claude/hooks/discovery_quality_gates.py --validate-file <DeliverablePath>`
  ...
  4. VERIFY: Does output confirm success?
     - If NO → **AUTONOMOUS SELF-CORRECTION**:
       - Analyze the error (e.g., "Missing JTBD IDs in persona-user.md")
       - **PROACTIVELY FIX THE ISSUE** (e.g., revise the file to add missing IDs)
       - **RERUN** the verification gate (Step 1-3)
       - DO NOT mark complete until success is achieved
...
EXECUTE verification_gate:
  ...
  IF failures.length > 0:
    ...
    ═══════════════════════════════════════════
    🔄 PROACTIVE SELF-CORRECTION (MANDATORY)
    ═══════════════════════════════════════════
    1. ANALYZE the failures (e.g., missing IDs, empty files).
    2. FIX the root cause by editing the affected files.
    3. RETRY the Verification Gate from Step 1.
    4. ONLY IF the fix is ambiguous or requires new high-level decisions, THEN ask:
       "how would you like to proceed?"
    ═══════════════════════════════════════════

    DO NOT proceed to "Update Progress" until fixes are verified.
