...
> **📁 OUTPUT STRUCTURE**: All skills must follow the folder structure defined in [PROTOTYPE_OUTPUT_STRUCTURE.md](PROTOTYPE_OUTPUT_STRUCTURE.md). This is the authoritative reference for file naming, directory organization, and output conventions.
...
> **🔗 DYNAMIC SKILL INVOCATION**: Skills can invoke other skills (including 3rdParty skills) using the `INVOKE_SKILL` pattern. See [PROTOTYPE_SKILL_INVOCATION.md](PROTOTYPE_SKILL_INVOCATION.md) for the full invocation framework and [PROTOTYPE_SKILL_REGISTRY.json](PROTOTYPE_SKILL_REGISTRY.json) for the skill registry.
...
> **See: [PROTOTYPE_VERIFICATION_GATE.md](PROTOTYPE_VERIFICATION_GATE.md) for full pattern**
