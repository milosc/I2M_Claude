...
description: Use when you need to implement features using test-driven development, ensuring every line of code is validated by tests written first with RED-GREEN-REFACTOR cycle.
model: sonnet
...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill tdd-methodology started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill tdd-methodology ended '{"stage": "utility"}'
---
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill tdd-methodology instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
