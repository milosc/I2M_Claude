...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill regulatory-affairs-head started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill regulatory-affairs-head ended '{"stage": "utility"}'
---
