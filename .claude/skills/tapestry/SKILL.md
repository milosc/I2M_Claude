---
name: weaving-content-plans
description: Use when you need to perform unified content extraction and create actionable implementation plans from URLs using the Tapestry workflow.
model: sonnet
...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill weaving-content-plans started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill weaving-content-plans ended '{"stage": "utility"}'
---
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill weaving-content-plans instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
