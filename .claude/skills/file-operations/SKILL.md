---
name: analyzing-file-metadata
allowed-tools: Bash, Glob, Grep
description: Use when you need to retrieve detailed file metadata, such as size, line counts, modification times, and content statistics, for analysis or comparison.
hooks:
  Stop:
    - hooks:
        - type: command
...
```bash
bash .claude/hooks/log-lifecycle.sh skill analyzing-file-metadata instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
