---
name: designing-combinatorial-tests
description: Use when you need to design comprehensive, efficient test suites using Pairwise Independent Combinatorial Testing (PICT) for complex requirements or code with multiple parameters.
model: sonnet
...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill designing-combinatorial-tests started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill designing-combinatorial-tests ended '{"stage": "utility"}'
---
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill designing-combinatorial-tests instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
