---
name: code-refactor
description: Use when you need to perform systemic code refactoring across multiple files, such as renaming identifiers, replacing deprecated patterns, or updating API calls consistently.
model: sonnet
...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill code-refactor started '{"stage": "implementation"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" skill code-refactor ended '{"stage": "implementation"}'
---
...
```bash
bash .claude/hooks/log-lifecycle.sh skill refactoring-code-bulk instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
