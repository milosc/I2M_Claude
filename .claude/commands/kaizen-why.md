...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /kaizen-why started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /kaizen-why ended '{"stage": "utility"}'
---
...
# 2. Log command start
bash .claude/hooks/log-lifecycle.sh command /kaizen-why instruction_start '{"stage": "implementation", "method": "instruction-based"}'
```
## Description
