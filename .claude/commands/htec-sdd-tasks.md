...
PRESENT 4 strategy questions to user:
    1. Decomposition approach:
        ...
        - Layer-by-layer (Infrastructure → Features → Integration)
...
IF EXISTS(JIRA_EXPORT):
    MODE = "reuse"
    LOG "✓ Detected ProductSpecs JIRA export - reusing tasks"
ELSE:
    MODE = "generate"
    LOG "⚠ No JIRA export found - generating tasks from module specs"
```
...
Module (MOD-MOB-INV-01)
    │
    ├── Infrastructure Task (T-001)
    │   └── Setup feature folder structure
    │
    ├── Type Definitions Task (T-002)
    │   └── Create TypeScript interfaces
    │
    ├── Service Layer Tasks (T-003..T-00N)
    │   ├── API client functions
    │   └── Business logic services
    │
    ├── Component Tasks (T-00N+1..T-00M)
    │   ├── UI components
    │   └── Custom hooks
    │
    ├── Screen Tasks (T-00M+1..T-00P)
    │   └── Page components
    │
    └── Integration Task (T-00P+1)
        └── Wire up and test
```
...
Files Created:
  ✓ traceability/task_registry.json (with pr_groups)
  ...
  ✓ Implementation_InventorySystem/tasks/TASK_INDEX.md
  ✓ Implementation_InventorySystem/tasks/T-001.md through T-047.md
...
Notes:
  • Both modes (reuse/generate) produce identical task_registry.json structure
  • Both go through same traceability pipeline (traceability/task_registry.json)
  • Reuse mode preserves full traceability from ProductSpecs Stage 3
```
