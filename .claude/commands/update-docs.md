...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /update-docs started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /update-docs ended '{"stage": "utility"}'
---
...
# 2. Log command start
bash .claude/hooks/log-lifecycle.sh command /update-docs instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
## Execution
...
8. **Launch `planning-tech-writer` agents for documentation (parallel)** (Sonnet or Opus models)
...
9. **Launch quality review agents (parallel)** (Sonnet or Opus models)
   - Launch `planning-tech-writer` agents again for quality review
   - Provide:
...
### Inline Documentation (Unit-Level)

For individual components, hooks, and services, use the **inline documentation pattern**:

**File Naming**: `<unit_name>_readme.md`
**Location**: Same folder as the source file

```
src/components/
├── KPICard.tsx
├── KPICard_readme.md          ← Documentation here
├── AlertFeed.tsx
└── AlertFeed_readme.md
```
...
- Any reusable code unit
- Complex business logic modules

**When to use project-level docs (`docs/` folder):**
- Getting started guides
- Architecture overviews
- Cross-cutting concerns
- API reference indices

**Command**: Use `/document <path>` for creating inline documentation.

See: `.claude/skills/Unit_Documentation/SKILL.md` for templates
See: `.claude/rules/inline-docs.md` for enforcement rules
