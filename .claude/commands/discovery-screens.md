...
hooks:
  ...
  Stop:
    - hooks:
        # VALIDATION: Check screen definitions document exists
        - type: command
          command: >-
        ...
        - type: command
          command: >-
            ...
            --file "ClientAnalysis_$(cat _state/discovery_config.json 2>/dev/null | grep -o '"system_name"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)/04-design-specs/screen-definitions.md"
            --contains "## Screen"
            --contains "### Components"
            --contains "### Data"
        # VALIDATION: Check screen registry exists
        - type: command
          command: >-
            uv run "$CLAUDE_PROJECT_DIR/.claude/hooks/validators/validate_files_exist.py"
        ...
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /discovery-screens ended '{"stage": "discovery", "validated": true}'
---
