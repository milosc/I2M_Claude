...
### Basic Usage (Without Orchestration Prompt)

```bash
# Simple feature request - uses default team composition and task structure
/plan_w_team "Add user authentication"

# Bug fix
/plan_w_team "Fix login timeout issue causing session drops"

# Refactoring task
/plan_w_team "Refactor database queries for better performance"
```

### Advanced Usage (With Orchestration Prompt)

The second argument lets you customize **how** the team is organized and executes the plan:

```bash
# Control team composition
/plan_w_team "Add user authentication" "Use only implementation agents, no planning agents"

# Control task granularity
/plan_w_team "Build payment system" "Break into small atomic tasks, each under 30 minutes"

# Maximize parallelism
/plan_w_team "Add search functionality" "Use vertical slicing, maximize parallel execution where possible"

# Security-focused execution
/plan_w_team "Add user authentication" "Assign quality-security-auditor for all auth tasks, run security review after each implementation task"

# Specific workflow pattern
/plan_w_team "Refactor database layer" "Use planning-code-explorer first to map architecture, then implementation-developer sequentially, batch review at end"

# Combined guidance
/plan_w_team "Build notification system" "Use reflexion-actor for initial implementation, reflexion-evaluator after each task, keep tasks coarse-grained, parallelize UI and API work"
```

### What the Orchestration Prompt Controls

| Aspect | Example Guidance |
|--------|------------------|
| **Team composition** | "Use only implementation agents" / "Include security-auditor for all tasks" |
| **Task granularity** | "Break into small atomic tasks" / "Keep tasks coarse-grained" |
| **Dependency structure** | "Maximize parallelism" / "Keep strictly sequential" |
| **Parallel/sequential** | "Run tests in parallel" / "Sequential TDD cycle" |
| **Review strategy** | "Batch review at end" / "Review after each task" |
| **Agent selection** | "Use reflexion loop for quality" / "Skip planning agents" |
...
TaskUpdate({
  ...
  owner: "implementation-developer"   // Assign to specific team member
})
```
...
### Available HTEC Team Members

The following agents are available in `.claude/agents/`:

| Agent ID | Role | Best For |
|----------|------|----------|
| `planning-code-explorer` | Code Analysis | Understanding existing codebase patterns and architecture |
| `planning-tech-lead` | Task Decomposition | Breaking features into atomic tasks with TDD specs |
| `implementation-developer` | Code Implementation | TDD-based feature implementation (RED-GREEN-REFACTOR) |
| `implementation-test-designer` | Test Design | Creating comprehensive test specifications |
| `implementation-test-automation-engineer` | E2E Testing | Playwright and integration test creation |
| `implementation-documenter` | Documentation | API docs, inline docs, README updates |
| `implementation-pr-preparer` | PR Creation | PR descriptions, review checklists |
| `quality-code-quality` | Code Review | SOLID principles, DRY, complexity analysis |
| `quality-security-auditor` | Security Review | OWASP vulnerabilities, security best practices |
| `quality-test-coverage` | Coverage Analysis | Test completeness, missing scenarios |
| `reflexion-actor` | Initial Solutions | Generate first-pass implementations |
| `reflexion-evaluator` | Quality Critique | Evaluate against acceptance criteria |
| `reflexion-self-refiner` | Polish & Optimize | Final improvements maintaining correctness |
...
4. Define Team Members - Use `ORCHESTRATION_PROMPT` (if provided) to guide team composition. Identify from `.claude/agents/*.md` or use `general-purpose`. Document in plan.
...
- Builder
  ...
  - Agent Type: <the subagent type of this builder, select from: planning-code-explorer, planning-tech-lead, implementation-developer, implementation-test-designer, implementation-test-automation-engineer, implementation-documenter, implementation-pr-preparer, quality-code-quality, quality-security-auditor, quality-test-coverage, reflexion-actor, reflexion-evaluator, reflexion-self-refiner, or general-purpose>
...
- **Agent Type**: <subagent type from available HTEC agents or general-purpose>
...
- **Agent Type**: <validator agent, e.g., quality-code-quality or reflexion-evaluator>
- **Parallel**: false
