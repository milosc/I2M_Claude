...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /kaizen-problem started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /kaizen-problem ended '{"stage": "utility"}'
---
...
# 2. Log command start
bash .claude/hooks/log-lifecycle.sh command /kaizen-problem instruction_start '{"stage": "implementation", "method": "instruction-based"}'
```
## Description
