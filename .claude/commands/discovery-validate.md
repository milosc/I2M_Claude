...
hooks:
  ...
  Stop:
    - hooks:
        # VALIDATION: Check VALIDATION_REPORT.md was created
        - type: command
          command: >-
        ...
        # VALIDATION: Run comprehensive discovery validation
        - type: command
          command: >-
            uv run "$CLAUDE_PROJECT_DIR/.claude/hooks/validators/validate_discovery_output.py"
            --system-name "$(cat _state/discovery_config.json 2>/dev/null | grep -o '"system_name"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)"
            --phase final
        # LOGGING: Record command completion
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /discovery-validate ended '{"stage": "discovery", "validated": true}'
---
