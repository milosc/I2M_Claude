...
hooks:
  PreToolUse:
    - matcher: "*"
      ...
      hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /reflect started '{"stage": "utility"}'
  Stop:
    - hooks:
        - type: command
          command: "$CLAUDE_PROJECT_DIR/.claude/hooks/log-lifecycle.sh" command /reflect ended '{"stage": "utility"}'
---
...
# 2. Log command start
bash .claude/hooks/log-lifecycle.sh command /reflect instruction_start '{"stage": "utility", "method": "instruction-based"}'
```
## Execution
